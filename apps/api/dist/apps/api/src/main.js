...
async function bootstrap() {
    ...
    app.use((req, res, next) => skipAuth(req) ? next() : urlencodedParser(req, res, next));
    const port = Number(process.env.PORT ?? 3001);
    await app.listen(port);
    common_1.Logger.log(`API listening on http://localhost:${port}`, "Bootstrap");
    common_1.Logger.log(`Better Auth handler: http://localhost:${port}/api/auth/*`, "Bootstrap");
    common_1.Logger.log(`CORS origin: ${webOrigin}`, "Bootstrap");
}
void bootstrap();
